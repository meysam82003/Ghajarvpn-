...
type Controller struct {
	...
	frontedMeekCDNScanRouteFound            int32
...
func (controller *Controller) isFrontedMeekCDNScanRouteFound() bool {
	return atomic.LoadInt32(&controller.frontedMeekCDNScanRouteFound) != 0
}

func (controller *Controller) markFrontedMeekCDNScanRouteFound() {
	atomic.StoreInt32(&controller.frontedMeekCDNScanRouteFound, 1)
}
...
func (controller *Controller) startEstablishing() {
	...
	controller.concurrentEstablishTunnelsMutex.Unlock()
	atomic.StoreInt32(&controller.frontedMeekCDNScanRouteFound, 0)
		...
		for {
			...
			if controller.config.AggressiveEstablishment && !wasServerAffinityCandidate {
				...
				for _, tunnelProtocol := range supportedProtocols {
					isFrontedMeekCDNScanProtocol :=
						protocol.TunnelProtocolUsesFrontedMeekCDN(tunnelProtocol)
					if isFrontedMeekCDNScanProtocol &&
						controller.isFrontedMeekCDNScanRouteFound() {
						continue
					}
					fanout := 1
					if isFrontedMeekCDNScanProtocol {
...
loop:
	for candidateServerEntry := range controller.candidateServerEntries {
		...
		if candidateServerEntry.preSelectedProtocol != "" &&
			protocol.TunnelProtocolUsesFrontedMeekCDN(candidateServerEntry.preSelectedProtocol) &&
			controller.isFrontedMeekCDNScanRouteFound() {
			continue
		}
