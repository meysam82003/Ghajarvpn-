...
function GhajarDirectPaymentLegacy($order_id, $image = 'images.jpg')
{
    ...
    if ($steppay[0] == "getconfigafterpay") {
        ...
        if (!is_array($marzban_list_get) || empty($marzban_list_get['name_panel']) || ($marzban_list_get['status']??'')!=='active' || !is_array($info_product)) {
            GhajarPurchaseSettlement::refund('gateway:'.$order_id);
            sendmessage($Balance_id['id'], 'سرویس قابل ارائه نیست؛ مبلغ پرداختی در کیف پول شما قرار گرفت.', $keyboard, 'HTML');return;
        }
        ...
        if (function_exists('nmPanelNationalEnabled') && nmPanelNationalEnabled($marzban_list_get)) {
            if (is_array($info_product) && nmStockCompleteBuyFromInventory($Balance_id['id'], $Balance_id, $marzban_list_get, $info_product, $get_invoice['id_invoice'], $username_ac, false, 'paid_national_buy')) {
                GhajarPurchaseSettlement::delivered('gateway:'.$order_id);
