...
object VpnCommandCoordinator {
    ...
    @Volatile var logger: (String) -> Unit = { GhajarLog.i("VpnCoordinator", it) }
    ...
    fun acceptBroadcast(report: Connection): Boolean {
        ...
        if (!accepted) logger("W broadcast rejected: report=$report contradicts in-flight $kind (age=${age}ms)")
        return accepted
    }

    /** Called when a real tunnel confirmation arrives; disarms its watchdog. */
    fun onTunnelConfirmed() {
        logger("tunnel confirmed connected")
        connectWatchdog?.cancel(); connectWatchdog = null
        lastCommand = null
    }
    ...
    fun onTunnelTeardown() {
        logger("tunnel teardown confirmed")
        connectWatchdog?.cancel(); connectWatchdog = null
        disconnectWatchdog?.cancel(); disconnectWatchdog = null
        lastCommand = null
    ...
    /**
     * Called when a command definitively fails (auth failure, no network, engine
     * crash, handshake error). Neither [onTunnelConfirmed] nor [onTunnelTeardown]
     * fits: the caller already moved VpnState to ERROR itself. This only disarms
     * both watchdogs and clears the stale-intent marker so the *next* broadcast
     * (e.g. the cleanup "disconnected" that usually follows an engine error) is
     * judged on its own merits instead of being filtered out as contradicting a
     * connect attempt that is, in fact, already over. Without this fix, a
     * broadcast arriving within STALE_MS of the error was silently dropped and
     * the connect/disconnect toggle could look stuck even though the engine had
     * already given up.
     */
    fun onTunnelFailed() {
        logger("tunnel failed / gave up")
        connectWatchdog?.cancel(); connectWatchdog = null
        disconnectWatchdog?.cancel(); disconnectWatchdog = null
        lastCommand = null
    }
