...
object StabilityTest {
    ...
    suspend fun run(testConfigJson: String, onProgress: (Phase, Double) -> Unit): Result? =
        coroutineScope {
            ...
            poller.cancel()
            val coreResult = parse(json) ?: return@coroutineScope null
            if (coreResult.uploadMbps > 0.01) return@coroutineScope coreResult

            // Some native cores finish without entering their upload phase. Do
            // a bounded HTTPS POST through the active tunnel instead of showing 0.
            onProgress(Phase.UPLOAD, 0.0)
            val fallback = SpeedTest.upload()
            if (fallback != null && fallback > 0.0) {
                onProgress(Phase.UPLOAD, fallback)
                coreResult.copy(uploadMbps = fallback)
            } else coreResult
        }
