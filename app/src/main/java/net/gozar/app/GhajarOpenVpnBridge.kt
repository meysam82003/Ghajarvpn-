    ...
    fun initialize(context: Context) {
        ...
        VpnStatus.addLogListener { item ->
            val level = when (item.logLevel) {
                ...
                de.blinkt.openvpn.core.VpnStatus.LogLevel.WARNING -> GhajarLogLevel.WARN
                de.blinkt.openvpn.core.VpnStatus.LogLevel.DEBUG,
                de.blinkt.openvpn.core.VpnStatus.LogLevel.VERBOSE -> GhajarLogLevel.DEBUG
                else -> GhajarLogLevel.INFO
            }
            val text = runCatching { item.getString(app) }.getOrDefault(item.toString())
            when (level) {
                GhajarLogLevel.ERROR -> GhajarLog.e("OpenVPN", text)
                GhajarLogLevel.WARN -> GhajarLog.w("OpenVPN", text)
                GhajarLogLevel.DEBUG -> GhajarLog.d("OpenVPN", text)
                else -> GhajarLog.i("OpenVPN", text)
            }
        }
                ...
                when (level) {
                    ConnectionStatus.LEVEL_CONNECTED -> {
                        ...
                        if (uuid != null) {
                            ...
                            VpnCommandCoordinator.onTunnelConfirmed()
                        }
                    }
