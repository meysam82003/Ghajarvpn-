...
final class PaymentStatusHandler extends BaseHandler
{
    ...
    public function handle(): void
    {
        ...
        $settlement=GhajarPurchaseSettlement::get('gateway:'.$orderId);
        $walletRefunded=($settlement['state']??'')==='refunded';
        if($walletRefunded){$isServiceReady=false;$service=null;$reasonFa='سرویس تحویل نشد؛ مبلغ پرداختی در کیف پول شما قرار گرفت.';}
        $payload = [
            'wallet_credited_only' => $walletRefunded,
            'settlement_status' => $settlement['state']??null,
            'order_id'         => $orderId,
